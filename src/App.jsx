...
// ── HORARIOS DE TAROTISTAS ────────────────────────────────────────────────────
// Cada tarotista tiene horario humano y horario IA
// Fuera de ambos → no disponible
const HORARIOS = {
  1: { humano:[9,17],  ia:[17,23] }, // Selene:  humana 9-17, IA 17-23
  2: { humano:[10,18], ia:[18,23] }, // Morgana: humana 10-18, IA 18-23
  3: { humano:[11,19], ia:[19,22] }, // Isadora: humana 11-19, IA 19-22
};

function getEstadoTarotista(id){
  const ahora = new Date().getHours();
  const h = HORARIOS[id];
  if(!h) return "offline";
  if(ahora >= h.humano[0] && ahora < h.humano[1]) return "humana";
  if(ahora >= h.ia[0]     && ahora < h.ia[1])     return "ia";
  return "offline";
}

// Para la web del cliente:
// "humana" o "ia" → disponible (puede escribir)
// "offline" → no disponible
function clientePuedeEscribir(id){ return getEstadoTarotista(id) !== "offline"; }
function esHorarioIA(id){ return getEstadoTarotista(id) === "ia"; }
...
// ── HOOK: estado de tarotistas (horario automático + override manual) ─────────
function useEstadoTarotistas(){
  const[overrides,setOverrides]=useState({});
  useEffect(()=>{
    // Escuchar overrides manuales del panel (solo cuando la tarotista fuerza un cambio)
  ...
  // Combinar horario automático con override manual
  // El override del panel solo aplica si está dentro del horario humano
  return overrides;
}

// Estado visible para el cliente
function useDisponibilidadCliente(){
  const[tick,setTick]=useState(0);
  useEffect(()=>{
    // Actualizar cada minuto para reflejar cambios de horario
    const t=setInterval(()=>setTick(p=>p+1),60000);
    return()=>clearInterval(t);
  },[]);
  return {
    puedeEscribir:(id)=>clientePuedeEscribir(id),
    esIA:(id)=>esHorarioIA(id),
    estado:(id)=>getEstadoTarotista(id),
  };
}
...
function Chat({usuario,t,onBack}){
  ...
  // Usar clave única email+tarotista para que siempre sea la misma conversación
  useEffect(()=>{
    if(!usuario.email) return;
    const clave = (usuario.email+"-"+t.id).replace(/[^a-z0-9]/gi,"-");
    setSesionId(clave);
    // Crear o actualizar la sesión en Firebase
    const sesionRef = ref(db,"conversaciones/"+clave);
    get(sesionRef).then(snap=>{
      if(!snap.exists()){
        // Nueva sesión
        set(sesionRef,{
          tarotistaid:t.id,
          ...
          inicio:Date.now(),
          creditos:creditosValidos(creditos).length,
        });
      } else {
        // Sesión existente — cargar mensajes previos
        const data = snap.val();
        if(data.mensajes){
          const msgs = Object.values(data.mensajes).sort((a,b)=>a.ts-b.ts);
          if(msgs.length>0) setMsgs(p=>[p[0],...msgs]);
        }
      }
    });
    // Escuchar mensajes nuevos en tiempo real
    const msgsRef = ref(db,"conversaciones/"+clave+"/mensajes");
    const unsub = onValue(msgsRef,snap=>{
      if(!snap.exists()) return;
      const msgs = Object.values(snap.val()).sort((a,b)=>a.ts-b.ts);
      setMsgs(p=>{
        // Solo actualizar si hay mensajes nuevos
        if(msgs.length > p.length-1) return [p[0],...msgs];
        return p;
      });
    });
    return()=>unsub();
...
function PaginaInicio({ir,abrirModal,m,creditos,onMostrarRecarga}){
  const disp=useDisponibilidadCliente();
  const isOnline=(t)=>disp.puedeEscribir(t.id);
  const getLabel=(t)=>disp.estado(t.id)==="humana"?"En línea":disp.estado(t.id)==="ia"?"Disponible":"No disponible";
  const getDot=(t)=>disp.estado(t.id)!=="offline"?"#34d399":"rgba(255,255,255,0.3)";
...
export default function App(){
  ...
  const handleOk=(form, t, creditosIniciales)=>{
    const userDb = buscarUsuario(form.email);
    if(!userDb){
